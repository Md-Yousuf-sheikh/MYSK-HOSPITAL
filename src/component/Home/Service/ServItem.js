...
const ServItem = (props) => {
    ...
    const { id, name, icon, subTitle } = props.item;
    // console.log(service);
    return (
        <div>
            <div>
                <div className="flex flex-col m-3 hover:shadow-lg py-5 px-3 overflow-hidden text-center">
                    ...
                    <Link service={service} to={`/ServicesDetail/${id}`} className="bg-blue-400 text-white hover:bg-transparent border-2 border-blue-400 hover:text-blue-500 shadow-md px-11 py-2">See More</Link>
                </div>
            </div>
        </div >
    );
